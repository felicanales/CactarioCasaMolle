...
| Backend API (`backend/`) | FastAPI 0.119 · Python 3.11                 | 8000       |
...
# Crear imagenes sin levantar contenedores
docker compose build

# Crear imagenes y levantar todo
...
El backend en Docker usa la imagen `python:3.11-slim` definida en `backend/Dockerfile`, por lo que no depende del Python instalado en Windows. El backend local de Docker no llama al backend de Railway: se conecta directamente a Supabase/R2 usando `backend/.env`. Si ese archivo apunta al mismo Supabase de produccion, las mutaciones locales tambien afectan los datos que vera Railway.
